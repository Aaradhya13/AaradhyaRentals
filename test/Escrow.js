...
describe('Escrow', () => {
    ...
    beforeEach(async () => {
        ...
        // Mint 
        let transaction = await realEstate.connect(seller).mint("https://aaradhya72.github.io/real-estate-json/1.json");
    ...
    describe('Cancellation', () => {
        ...
        it('Should return NFT ownership to seller if inspection not passed', async () => {
            expect(await realEstate.ownerOf(1)).to.be.equal(escrow.address);
            await escrow.connect(seller).cancelSale(1);
            expect(await realEstate.ownerOf(1)).to.be.equal(seller.address);
        });

        it('Should refund buyer if inspection not passed', async () => {
